...
set PATH="T:\Program Files\7-Zip";%PATH%
:: ---------------------------------------------------------------
...
::pushd data\po\locales\de\LC_MESSAGES
::if exist dbase.mo ( del dbase.mo )
::msgfmt -o dbase.mo dbase.po
::if errorlevel 1 (
...
:locales_copy
copy /y data\po\locales.zip %DIST%\data\locales.zip
if errorlevel 1 (
    echo error copy locales.zip
    goto have_error
)
::styles_zip
::set "SRC=data\po\styles.zip"
...
:styles_copy
copy /y data\po\styles.zip %DIST%\data\styles.zip
if errorlevel 1 (
