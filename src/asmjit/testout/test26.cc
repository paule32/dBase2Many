...
int main() {
    ...
    a.mov(x86::r11, x86::qword_ptr(x86::r12, offsetof(JitContext, arrays_vars)));
    a.movsxd(x86::rax, x86::eax);
    a.add(x86::rax, x86::r11); // @array[index]
    ...
    a.mov(x86::dword_ptr(x86::rax), x86::ebx); // p^ :=
    a.xor_(x86::ebx, x86::ebx); // linear array index
    a.mov(x86::eax, 0);
    ...
    a.mov(x86::rax, x86::qword_ptr(x86::r12, offsetof(JitContext, pointr_vars)));
    a.mov(x86::rax, x86::qword_ptr(x86::rax, 0)); // p
    a.mov(x86::eax, x86::dword_ptr(x86::rax)); // p^
    a.mov(x86::ecx, x86::eax);
    a.mov(x86::rax, imm((uint64_t)&_jit_print_int));
    a.sub(x86::rsp, 32); // Windows x64 shadow space
    a.call(x86::rax);
    ...
    asm_out << "arrays_vars: times 40 db 0\n";
    ...
    if (!write_formatted_asm_file(
        ...
        std::cerr << "Could not write ASM file: test26.asm" << std::endl;
    }
