...
static const char str_0[] = "Length: ";
...
int main() {
    ...
    a.mov(x86::rcx, x86::rax);
    a.mov(x86::rax, imm((uint64_t)&_jit_dynstring_length));
    a.sub(x86::rsp, 32); // Windows x64 shadow space
    a.call(x86::rax);
    a.add(x86::rsp, 32);
    ...
    if (!write_formatted_asm_file(
        ...
        std::cerr << "Could not write ASM file: test49.asm" << std::endl;
    }
