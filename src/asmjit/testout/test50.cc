...
static const char str_1[] = "Y";
static const char str_2[] = "Z";
static const char str_3[] = "ABC";
...
int main() {
    ...
    a.mov(x86::rax, imm((uint64_t)&_jit_dynstring_setlength));
    ...
    a.mov(x86::rax, imm((uint64_t)&_jit_string_range_error));
    a.sub(x86::rsp, 32); // Windows x64 shadow space
    a.call(x86::rax);
    a.add(x86::rsp, 32);
    ...
    a.call(x86::rax);
    a.add(x86::rsp, 32);
    a.push(x86::rax); // save left string
    a.mov(x86::rax, x86::qword_ptr(x86::r12, offsetof(JitContext, string_vars)));
    a.mov(x86::rax, x86::qword_ptr(x86::rax, 0)); // S
    a.mov(x86::rdx, x86::rax); // right string
    a.pop(x86::rcx);           // left string
    a.mov(x86::rax, imm((uint64_t)&_jit_dynstring_concat));
    a.sub(x86::rsp, 32); // Windows x64 shadow space
    a.call(x86::rax);
    a.add(x86::rsp, 32);
    ...
    asm_out << "_str_1 db \"Y\", 0\n";
    asm_out << "_str_2 db \"Z\", 0\n";
    asm_out << "_str_3 db \"ABC\", 0\n";
    ...
    if (!write_formatted_asm_file(
        ...
        std::cerr << "Could not write ASM file: test50.asm" << std::endl;
    }
