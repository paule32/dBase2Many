...
static const char str_1[] = "before nil access";
static const char str_2[] = "P";
static const char str_3[] = "Nil pointer error: P";
static const char str_4[] = "after nil access";
static const char str_5[] = "except block";
static const char str_6[] = "done";
...
int main() {
    ...
    a.mov(x86::qword_ptr(x86::r11, 0), x86::rax); // P
    ...
    a.call(x86::rax);
    a.add(x86::rsp, 32);
    Label except_1 = a.new_label();
    Label endtry_2 = a.new_label();
    ...
    a.mov(x86::rax, x86::qword_ptr(x86::rax, 0)); // P
    Label ptr_not_nil_3 = a.new_label();
    a.test(x86::rax, x86::rax);
    a.jnz(ptr_not_nil_3);
    a.mov(x86::rcx, x86::r12); // ctx
    a.mov(x86::rdx, imm((uint64_t)str_3));
    a.mov(x86::rax, imm((uint64_t)&_jit_set_exception));
    a.sub(x86::rsp, 32); // Windows x64 shadow space
    a.call(x86::rax);
    a.add(x86::rsp, 32);
    a.jmp(except_1);
    a.bind(ptr_not_nil_3);
    a.mov(x86::dword_ptr(x86::rax), x86::ebx); // P^.Value :=
    ...
    symbols.add(std::to_string((uint64_t)&jit_runtime_error), "_jit_runtime_error");
    ...
    labels.add("L0", "except_1");
    labels.add("L1", "endtry_2");
    ...
    asm_out << "extern _jit_set_exception\n";
    ...
    asm_out << "_str_1 db \"before nil access\", 0\n";
    asm_out << "_str_2 db \"P\", 0\n";
    asm_out << "_str_3 db \"Nil pointer error: P\", 0\n";
    asm_out << "_str_4 db \"after nil access\", 0\n";
    asm_out << "_str_5 db \"except block\", 0\n";
    asm_out << "_str_6 db \"done\", 0\n";
    ...
    if (!write_formatted_asm_file(
        ...
        std::cerr << "Could not write ASM file: test48.asm" << std::endl;
    }
