...
from compiler.frontend.lisp.generator  import *
...
def main() -> int:
    ...
    try:
        ...
        if ext.lower() in [".lisp", ".lsp", ".l"]:
            CDATA.args_compilermode = args.compilermode
            source_file = name + ext
        else:
            CDATA.args_compilermode = args.compilermode
            if args.compilermode in ["l", "lsp", "lisp"]:
                if not ext or len(ext) < 2:
                    source_file = name + ".lisp"
            else:
        ...
        # -----------------------------------------
        # 2. lexical analyse LISP file ...
        # -----------------------------------------
        lexer   = LispLexer(stream)
        ...
        # -----------------------------------------
        generator = LispGenerator(backend, writer=target_obj)
        ...
        if CDATA.args_target.lower() in ["dos", "dos16"]:
            ...
            if CDATA.args_backend.lower() in ["exe", "exefile"]:
                if not generator.main_emitted:
                    ...
                    #    "LISP source does not contain a 'program:' refinement"
                    #)
                outfile = Path(CDATA.exe_file)
                check   = ['j','y']
                if outfile.exists():
