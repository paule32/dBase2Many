...
LBB0_13:
    test	eax, eax
    je	LBB0_15
    mov	ecx, dword [esp + eax + 28]
    bswap	ecx
    mov	dword [esp + eax + 188], ecx
    add	eax, 4
    jmp	LBB0_13
LBB0_15:
    push	57
...
LBB0_18:
    test	ecx, ecx
    je	LBB0_20
    movzx	edx, byte [esp + ecx + 188]
    mov	esi, edx
    shr	esi, 4
    mov	bl, byte [esi + _hex_alloc_lo]
    mov	byte [eax + 2*ecx + 56], bl
    and	edx, 15
    mov	dl, byte [edx + _hex_alloc_lo]
    mov	byte [eax + 2*ecx + 57], dl
    inc	ecx
    jmp	LBB0_18
LBB0_20:
    mov	byte [eax + 56], 0
